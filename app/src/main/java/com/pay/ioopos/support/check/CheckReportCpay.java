...
public class CheckReportCpay extends CheckAbstract {
    ...
    /**
     * 检查艾博世平台配置参数与腾讯云支付参数配置是否相对应
     * 这个需要用户去确认，因为艾博世平台配置的是微信商户号，设备从腾讯云支付接口拿到的是腾讯云支付平台的商户号
     * @author  Moyq5
    ...
    private void conifrmParams() {
        ...
        info("检查上报：请确认以下信息是否与腾讯云支付平台配置一致，是请按1，否则请按2，忽略请按3");

        StringBuilder sb = new StringBuilder();
        sb.append("┌─────┬──────┬────────────┐\n");
        sb.append("\t                                    微信                                 云支付\n");
        sb.append("├─────┼──────┼────────────┤\n");
        sb.append("\t商户号            "+sdkParams.getMchId()+"      "+store.getOutMchId() + "\n");
        if (null != store.getOutSubMchId() || null != sdkParams.getSubMchId()) {
            sb.append("├─────┼──────┼────────────┤\n");
            sb.append("\t子商户号        "+sdkParams.getSubMchId()+"     "+store.getOutSubMchId() + "\n");
        }
        sb.append("└─────┴──────┴────────────┘");
        info(sb.toString());

        addSpeak("请核对腾讯云支付参数是否正确，是请按1，否则请按2，忽略请按3");
        timeCallback = () -> addSpeak("请核对腾讯云支付参数是否正确，是请按1，否则请按2，忽略请按3");
        KeyInfoListener listener = keyInfo -> {
            timeCallback = null;
            switch (keyInfo) {
                case KEY_NUM_1:
                    info("检查上报：参数已确认");
