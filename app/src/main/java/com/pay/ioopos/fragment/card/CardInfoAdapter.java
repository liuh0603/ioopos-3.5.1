 ...
 * 刷卡记录
 * @author   Moyq5
 * @since  2021/11/04 10:08
 */
public class CardInfoAdapter extends ArrayAdapter<CardOrder> {
    private final Context context;
    private final int resource;

    public CardInfoAdapter(Context context, int resource, List<CardOrder> orders) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        CardOrder order = getItem(position);
        ...
        addTime.setText(new SimpleDateFormat("MM-dd HH:mm:ss").format(new Date(order.getOrderTime() * 1000)));
