 ...
 * @since 2021-07-23
 */
public class WxOfflineIniWorker extends Worker {
    private static final Lock lock = new ReentrantLock();

    public static final String TAG = WxOfflineIniWorker.class.getName();
    /**
     * 初始化周期，单位：毫秒
     */
     ...
     * 微信刷脸app版本信息
     */
    private static Map<String, Object> sdkInfo;

    /**
     * 获取rawData及其后续初始化流程
     */
    private static final Runnable rawDataRunnable = () -> getWxRawData(() -> getWxMerch(() -> {
        if (!isFirstTimeSuccess) {
            updateStatus(STATUS_SUCCESS, "");
            return;
        }
        isFirstTimeSuccess = false;
        preloadEnv(() -> manualUpdateFaceDatas(() -> updateStatus(STATUS_SUCCESS, "")));
    }));
    ...
    public WxOfflineIniWorker(@NonNull @NotNull Context context, @NonNull @NotNull WorkerParameters workerParams) {
    ...
    private static void preloadEnv(Runnable callback) {
        updateStatus("环境装载...");
        try {
            MyWxPayFace.getInstance().preloadSdkEnv(params, new IWxPayfaceCallback() {
                @Override
                public void response(Map info) throws RemoteException {
                    if (info == null) {
                        unavailable("preloadSdkEnv");
                        return;
                    }
                    String code = (String) info.get("return_code");
                    if (code == null || !code.equals("SUCCESS")) {
                        unavailable("preloadSdkEnv：%s", info.get("return_msg"));
                        return;
                    }
                    TaskFactory.execute(callback);
                }
            ...
            Log.e(TAG, "微信刷脸 preloadEnv 失败: ", e);
            unavailable("preloadEnv 失败: %s", e.getMessage());
        }
    }

    private static void manualUpdateFaceDatas(Runnable callback) {
        updateStatus("信息更新...");
        try {
            MyWxPayFace.getInstance().manualUpdateFaceDatas(params, new IWxPayfaceCallback() {
                ...
                public void response(Map info) throws RemoteException {
                    if(info == null) {
                        unavailable("manualUpdateFaceDatas");
                        return;
                    }
                    String code = (String) info.get("return_code");
                    if(null == code  || !code.equals("SUCCESS")) {
                        unavailable("manualUpdateFaceDatas：%s", info.get("return_msg"));
                        return;
                    }
                    TaskFactory.execute(callback);
                }
            ...
            Log.e(TAG, "微信刷脸 manualUpdateFaceDatas 失败: ", e);
            unavailable("manualUpdateFaceDatas 失败: %s", e.getMessage());
        }
    }
    ...
    private static void updateStatus(int sta, String format, Object... args) {
        ...
        if (status == STATUS_SUCCESS) {
            ...
            loadSdkInfo();
            // 从服务端同步刷脸用户基本信息到本地
            WorkerFactory.enqueueWxUserLoadOneTime();
        }
